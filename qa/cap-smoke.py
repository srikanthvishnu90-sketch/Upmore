#!/usr/bin/env python3
"""Smoke test the 3 new capability paths on production."""
...
def api(method, url, key, body=None, bearer=None, tries=6):
    ...
    for i in range(tries):
        tmp = tempfile.mktemp(prefix="cap_")
        cmd = ["curl", "-s", "-o", tmp, "-w", "%{http_code}", "-X", method, url,
               "--max-time", "90", "-H", "apikey: " + key,
...
email = f"cap-{uuid.uuid4().hex[:8]}@upmore-qa.local"
...
api("POST", PROJ + "/rest/v1/profiles", SVC, {"id": uid, "display_name": "CAP"})
...
for q in ["can I make money on polymarket? is it a good way to make extra cash?",
          "make me $20",
          "what are some good stocks you think i should invest in, and why"]:
    st, ms, r = chat(q)
    print("=" * 70)
    print("Q:", q, f"[{st} {ms}ms]")
    print(r[:2200])
    time.sleep(1)
