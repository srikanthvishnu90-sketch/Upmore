#!/usr/bin/env python3
"""Quick re-probe: make-me-$20 with 14 verified routes live."""
...
def api(method, url, key, body=None, bearer=None):
    ...
    tmp = tempfile.mktemp(prefix="rp_")
...
email = f"rp-{uuid.uuid4().hex[:8]}@upmore-qa.local"
...
api("POST", PROJ + "/rest/v1/profiles", SVC, {"id": uid, "display_name": "RP"})
st, tok = api("POST", PROJ + "/auth/v1/token?grant_type=password", ANON, {"email": email, "password": pw})
jwt = json.loads(tok)["access_token"]

t0 = time.time()
st, body = api("POST", PROJ + "/functions/v1/agent-chat", ANON, {"message": "make me $20"}, bearer=jwt)
